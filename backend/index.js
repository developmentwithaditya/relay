...
app.use(cors()); // This handles standard API requests

const server = http.createServer(app);

// --- FIX: Added specific CORS configuration for Socket.IO ---
const io = new Server(server, {
  cors: {
    origin: "http://localhost:5173", // Allow connections from our frontend
    methods: ["GET", "POST"]
  }
});
...
app.get('/api/me', authMiddleware, async (req, res) => {
  try {
    const user = await User.findById(req.user.userId)
      .select('-password')
      .populate('partnerId', 'email role')
      .populate('pendingRequests', 'email role');
    res.json(user);
  } catch (error) {
...
io.on('connection', (socket) => {
  ...
  socket.on('send_order', async ({ items, senderId }) => {
    try {
      const sender = await User.findById(senderId);
      if (!sender || !sender.partnerId) return console.log('Sender or partner not found.');
      
      const partnerSocketId = userSockets[sender.partnerId];
      if (partnerSocketId) {
        const newOrder = new Order({ items, senderId, receiverId: sender.partnerId });
        await newOrder.save();
  ...
  socket.on('acknowledge_order', async ({ orderId, receiverId }) => {
    try {
        const order = await Order.findByIdAndUpdate(orderId, { status: 'acknowledged' });
        if (!order) return;
...
server.listen(PORT, () => {
  console.log(`🚀 Backend server with Connections running at http://localhost:${PORT}`);
});
